...
function setLoading(loading) {
  ...
  status.textContent = loading ? 'AI 正在根据你的背景生成院校名单与推荐理由，请稍候，无需重复提交。' : '';
  ...
  submitBtn.innerHTML = loading
    ? '<span class="spinner"></span>AI 正在生成选校建议...'
    : '让 AI 生成选校建议 <span aria-hidden="true">→</span>';
}
...
function renderTiers(tiers) {
  ...
  for (const tier of tiers) {
    ...
    for (const school of tier.schools) {
      ...
      reason.textContent = school.reason || '留资后查看 AI 推荐理由';
